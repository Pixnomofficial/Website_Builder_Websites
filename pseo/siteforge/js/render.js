...
(async function () {
  ...
  try {
    ...
    // Override fetch inside the iframe to redirect publish calls to our siteforge endpoint
    const uuid = JSON.stringify(ROW_ID);
    const publishOverride =
      '<script>(function(){' +
        'var _origFetch=window.fetch;' +
        'window.fetch=function(url,opts){' +
          'if(typeof url==="string"&&url.indexOf("/api/publish/cms")>-1){' +
            'var body=opts&&opts.body?JSON.parse(opts.body):{};' +
            'return _origFetch("' + SERVER + '/website-builder/api/cms/siteforge-publish",{' +
              'method:"POST",headers:{"Content-Type":"application/json"},' +
              'body:JSON.stringify({uuid:' + uuid + ',contentJson:body.contentJson||"{}"})' +
            '});' +
          '}' +
