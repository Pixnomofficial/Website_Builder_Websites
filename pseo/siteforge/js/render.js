...
(async function () {
  ...
  try {
    ...
    // Build override script: apply saved _cms_overrides before the CMS editor reads from DOM
    let overridesScript = "";
    if (biz._cms_overrides) {
      const ov = JSON.stringify(biz._cms_overrides);
      overridesScript =
        '<script>window.__CMS_OVERRIDES__=' + ov + ';</script>' +
        '<script>document.addEventListener("DOMContentLoaded",function(){' +
          'var d=window.__CMS_OVERRIDES__;' +
          'if(!d)return;' +
