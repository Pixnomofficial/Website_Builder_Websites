...
const SERVICES_MAP = {
  ...
  'window-cleaning': ['Residential Window Cleaning', 'Commercial Window Cleaning', 'High-Rise Window Cleaning', 'Interior & Exterior Cleaning', 'Screen Cleaning & Repair', 'Window Track & Sill Cleaning', 'Hard Water Stain Removal', 'Skylight Cleaning', 'Solar Panel Cleaning', 'Gutter Cleaning', 'Pressure Washing Add-On', 'Post-Construction Window Cleaning'],
  // legacy entries kept
...
const REVIEWS_MAP = {
  ...
  'window-cleaning': [
    { name: 'Harold Whitfield', text: 'Every window in our two-story home is streak-free and crystal clear. They even cleaned the tracks and sills without being asked. Outstanding.', stars: 5, date: '1 week ago' },
    { name: 'Beatrice Lindqvist', text: 'Years of hard water staining on our shower and patio glass — completely gone. I honestly thought those panes would need replacing.', stars: 5, date: '3 weeks ago' },
    { name: 'Desmond Achebe', text: 'They handle our office building quarterly. Fully insured, properly equipped for the upper floors, and never disrupt our workday.', stars: 5, date: '1 month ago' },
    { name: 'Rosalind Byrne', text: 'Post-renovation window cleaning removed all the paint specks and construction film. The house finally looks finished.', stars: 4, date: '5 weeks ago' },
    { name: 'Emmett Kowalski', text: 'Skylights I could never reach are spotless again, and the difference in natural light is genuinely noticeable. Fair price too.', stars: 5, date: '2 months ago' },
    { name: 'Ingrid Fontaine', text: 'Signed up for the twice-yearly plan. Same crew each visit, always on time, and they take real care around the landscaping.', stars: 5, date: '2 months ago' },
  ],
  // legacy reviews
  electrician_legacy: [
...
const SERVICE_IMAGES = {
  ...
  'window-cleaning': { hero: 'https://images.pexels.com/photos/37440103/pexels-photo-37440103.jpeg', about: 'https://images.pexels.com/photos/713297/pexels-photo-713297.jpeg?auto=compress&cs=tinysrgb&w=800&h=600&fit=crop', portfolio: ['https://images.pexels.com/photos/31983866/pexels-photo-31983866.jpeg?auto=compress&cs=tinysrgb&w=800&h=600&fit=crop', 'https://images.pexels.com/photos/17041922/pexels-photo-17041922.jpeg?auto=compress&cs=tinysrgb&w=800&h=600&fit=crop', 'https://images.pexels.com/photos/31435403/pexels-photo-31435403.jpeg?auto=compress&cs=tinysrgb&w=800&h=600&fit=crop', 'https://images.pexels.com/photos/7641351/pexels-photo-7641351.jpeg?auto=compress&cs=tinysrgb&w=800&h=600&fit=crop'] },
...
const SERVICE_LOGO = {
  ...
  'window-cleaning': { icon: '🪟', label: 'Window Cleaning' },
  // legacy
...
function getDefaultTagline(type) {
  const tags = {
    ...
    'window-cleaning': "Streak-Free Views, Every Time — Professional Window Cleaning You Can See",
...
.sf-marquee{overflow:hidden;padding:14px 0;border-top:1px solid ${borderColor};border-bottom:1px solid ${borderColor};background:${isDark ? 'rgba(255,255,255,0.02)' : 'rgba(0,0,0,0.02)'}}
.sf-marquee-track{display:flex;gap:48px;animation:sfMarquee 25s linear infinite;white-space:nowrap}
.sf-marquee-track span{font-size:32px;color:${MU};font-weight:500;letter-spacing:0.5px}
.sf-marquee-dot{margin:0 4px}
...
<div id="page-home" class="sf-page">
  ...
  ${showMarquee ? `<div class="sf-marquee"><div class="sf-marquee-track">${marqueeItems.map(s => `<span>${s}</span><span class="sf-marquee-dot">&bull;</span>`).join('')}${marqueeItems.map(s => `<span>${s}</span><span class="sf-marquee-dot">&bull;</span>`).join('')}</div></div>` : ''}
  <div class="section">
    <div class="sec-in">
...
async function downloadZIP() {
  ...
  // CMS: tag index.html and add content.json + cms-editor.js
  if (parsed.htmlPages['index.html']) {
    const { tagged, contentJson } = tagHTMLForCMS(parsed.htmlPages['index.html']);
    root.file('index.html', tagged);
    root.file('content.json', JSON.stringify(contentJson, null, 2));
    root.file('cms-editor.js', CMS_EDITOR_SCRIPT);
  }
...
async function generateAll30() {
  ...
  for (let i = 0; i < DESIGNS.length; i++) {
    ...
    try {
      ...
      if (parsed.htmlPages['index.html']) {
        const { tagged, contentJson } = tagHTMLForCMS(parsed.htmlPages['index.html']);
        subfolder.file('index.html', tagged);
        subfolder.file('content.json', JSON.stringify(contentJson, null, 2));
        subfolder.file('cms-editor.js', CMS_EDITOR_SCRIPT);
      }
    } catch (err) {
